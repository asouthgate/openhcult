...
import numpy as np
import pandas as pd
from hcultinf.detection import (
    DynamicIntervalInfo,
    merge_intervals,
    lerp_thresholds,
    find_regions_with_hysteresis_adaptive_thresh,
    compute_time_weighted_ewma,
...
def test_hysteresis_negative_direction_and_trailing_active():
    ...
    regions = find_regions_with_hysteresis_adaptive_thresh(
        times, val, trigger, release, direction=-1
    )
...
def test_ewma_time_gap_sensitivity():
    times = pd.to_datetime(
        ["2026-01-01 10:00", "2026-01-01 10:10", "2026-01-01 11:50"]
    ).values
...
def test_segment_detector_integration_flow():
    ...
    values = np.concatenate(
        [np.full(40, 3000.0), np.linspace(3000, 1000, 20), np.full(60, 1000.0)]
    )
