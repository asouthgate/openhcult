...
def _get_non_duplicate_events(
    ...
    for sensor, points in series.items():
        ...
        ded = SegmentDetector(
            times, values, emwa_tau_minutes, trigger_threshold, release_threshold
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    series, observations = fetch_data(ctrl_url, start_utc, end_utc, limit=args.limit)
    if not series:
