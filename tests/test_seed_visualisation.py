...
def test_seed_visualisation_data(db_conn):
    ...
    plants_rows = list(database.fetch_plants(db_conn, include_sensors=False))
    plant_names_in_db = {row["plant_name"] for row in plants_rows}
    for p in _PLANTS:
        assert p["plant"] in plant_names_in_db

    # Phase 2: register devices + assign sensors
    for s in _SENSORS:
    ...
    mappings = list(database.fetch_plant_sensors(db_conn))
    assert len(mappings) == len(_SENSORS)

    # Phase 3: simulate moisture per plant, derive per-sensor readings, insert
    plant_fc_data = {}
    for p in _PLANTS:
    ...
    # Water observations: once per plant (not per sensor)
    inserted_plants = set()
    for s in _SENSORS:
    ...
    # Phase 3 assertions: verify readings come back via API
    for p in _PLANTS:
        series = request_json(f"/timeseries?limit=100&plant={p['plant']}")
        assert series["count"] > 0

    # Verify all 3 sensors have readings
    for s in _SENSORS:
        series = request_json(
